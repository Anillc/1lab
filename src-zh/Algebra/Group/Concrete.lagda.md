...
with the [[delooping]] of an abstract group; note that, here, the delooping _is_ the
...
_all_ concrete groups arise as deloopings.
...
The notion of group _homomorphism_ between two groups $G$ and $H$ gets translated
to, on the "concrete" side, [[_pointed_ maps]] $\B{G} \to^\bullet \B{H}$.

The pointedness condition ensures that these maps behave like abstract group
homomorphisms; in particular, that they form a _set_.
...
we need one more thing: that $p$ extends $g$ on the _right_. We get this essentially
...
a family that asserts that $p_x$ agrees with $f$ _all the way_, not just on loops:
