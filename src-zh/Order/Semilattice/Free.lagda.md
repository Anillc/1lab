...
This module provides an _impredicative_ construction of the free [[join
...
since _every subset is a join of singletons_.
...
semilattice is a _commutative, idempotent_ monoid.
...
taking a join, so it suffices to show that $g$ is _also_ computed via a
...
finitely-indexed joins, so $g$ _must_ be computed as a join over the
