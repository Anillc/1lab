...
\cJ \to \cE$ would give a $\cJ$-shaped diagram in $\cE$ _lying over_ a
...
Furthermore, such liftings commute _extremely strictly_. Not only are
the two functors equal, the action on objects and morphisms are
_definitionally_ equal! This highlights the utility of the theory
...
are precisely the natural transformations that _definitionally_ lie over
their bases.
...
As noted earlier, the total category of $\cE^{\cJ}$ _is_ the functor
