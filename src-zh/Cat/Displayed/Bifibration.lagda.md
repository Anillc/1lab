...
Note that a bifibration is _not_ the same as a "profunctor valued in
...
construct natural isomorphism between $\cE\_{y}(u\_{_}(-),-)$ and
$\cE\_{x}(-,u^{_}(-))$. However, we have already shown that
$\cE\_{y}(u\_{_}(-),-)$ and $\cE\_{x}(-,u^{_}(-))$ are both naturally
isomorphic to $\cE\_{u}(-,-)$[^proof], so all we need to do is compose these
...
natural isomorphism $\cE_{u}(x',-) \simeq \cE_{x}(x', u^{_}(-))$, by the
universal property of [[cartesian lifts]]. If $u^{_}$ additionally has a
