...
vertical retractions]. The cartesian lift $f^{_}$ of $f$ is obviously
cartesian, so it suffices to show that there is a vertical retraction
$x^{_} \to x'$. To construct this retraction, we shall factorize $f'$
over $f \cdot id$; which yields a vertical morphism $i^{_} : x' \to x^{_}$.
By our hypotheses, $i^{_}$ is invertible, and thus is a retraction.
What remains to be shown is that the inverse to $i^{_}$ factors
$f'$ and $f^{_}$; this follows from the factorisation of $f'$ and
the fact that $i^{_}$ is invertible.
