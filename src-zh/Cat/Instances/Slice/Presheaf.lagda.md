...
element $(x, s)$ in $\int P$, the fibre $\eta\_x^\*(s)$ is a set. But why
...
In fact, since we have $\Sets = \psh(_)$, that latter equivalence is a
special case of the one constructed here --- where in the calculation
below, $c$ denotes the constant presheaf $_ \mapsto S$. The category of
