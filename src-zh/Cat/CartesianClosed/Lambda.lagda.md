...
We will implement our semantics using _normalisation by evaluation_, by
...
presheaves on a category of _inclusions_, where the objects are contexts
and the maps $\Gamma \to \Delta$ indicate that, starting from $\Gamma$,
you can get to $\Delta$ by _dropping_ some of the variables, and keeping
...
Therefore, there are expressions that we can not _give a value to_, but
...
variables together with a stack of _pending eliminations_ (applications
...
We would like to write a map of type `Expr Γ τ → Nf Γ τ`. However, _by
design_, this is not straightforward: the type of normal forms is...
$\beta$-normal.^\[It is not, however, $\eta$-long.] However, note that,
since both `Nf` and `Ne` are "presheaves" on the "category of
renamings", we can use the Cartesian closed structure _of presheaves_ to
...
Then, as long as we can _reify_ these presheaves back to normal forms,
