...
they are the _least fixpoints_ of functors. This result is known as Lambek's
...
objects. It is now time to see _why_ this is the case. Suppose that
...
The left adjoint takes each object $A$ to an object $F^{_}(A)$ that is equipped
with an $F$-algebra $\mathrm{roll} : \cC(F(F^{_}(A)),F^{\*}(A))$.
...
We can also view a monad algebra $\alpha$ as a _morphism_ of $F$-algebras,
...
This diagram states that we should be able to eliminate a $F(F^_(A))$
either by rolling in the $F$ and passing the resulting $F^_(A)$ off
to the monad algebra, or by eliminating the inner $F^\*(A)$ first,
and _then_ evaluating the remaining $F(A)$. Intuitively, this is quite
...
if we have _all_ free $F$-algebras. This is a rather strong condition
