...
\cD$: an object $F^_(d) : \cC$ together with a specified isomorphism
$F^_(d) \cong d$.
...
part, the splitting gives us isomorphisms $F^_(x) \cong x$ and $F^_(y)
\cong y$, so that we may form the composite $F^_(x) \to x \to y \to
F^_(y)$; Fullness then completes the construction.
...
a morphism $x \to F^_F(x)$ --- where, recall, the notation $F^_(x)$
...
_preserve invertibility_, in the sense that the adjunct of an isomorphism
