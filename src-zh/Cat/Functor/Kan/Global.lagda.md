...
is a universal solution $\Lan\_p(F)$ to extending $F$ to a functor $C'
\to D$. In particularly happy cases (e.g. when $C$ is [small] and $D$ is
[cocomplete]), the left Kan extension $\Lan\_p(F)$ along $p$ exists for
**any** $F$; When this happens, the assignment $F \mapsto \Lan\_p(F)$
