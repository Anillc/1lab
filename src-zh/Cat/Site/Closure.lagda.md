...
In reality, the first few of these properties don't even _need_ a site:
...
_itself_, local: If $A$ is a sheaf for a sieve $R$, and we have some
other sieve $S$ which we want to show $A$ is a sheaf for, it (almost)
suffices to show that A is an $f^_(S)$-sheaf for every $f \in R$. If $R$
and $S$ are not part of a site, then this condition does not suffice: we
also require that $A$ be $f^_(R)$-separated, for every $f \in S$.
...
pulled back to give a patch $f^_(p)$ on $f^_(S)$, for any $f \in R$;
...
that $A$ is $f^_(R)$-local comes in (for $f \in S$): we can reduce this
to showing $A(gf)(s) = A(g)(p(f))$, with $fg \in R$. We would almost be
in trouble again, since $p''(fg)$ is only characterised $S$-locally, but
we can appeal to separatedness _again_, this time at $(fg)^_(S)$.  This
_finally_ allows the calculation to go through:
...
for any _pullback of_ a $\widehat{J}$-covering sieve".
