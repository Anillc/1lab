...
morphism $f : \Gamma \to A$ as an _$A$-term in context $\Gamma$_, the
notion of **exponential object** captures what it means for an object to
interpret _function types_. An exponential object for $A$ and $B$ is an
...
We refer to an object $B : \cC$ as exponentia_ble_ if, for every other
