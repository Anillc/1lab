...
A$". Note that the judgement $\vdash A$​ is a part of the _meta_theory,
...
point $y \in B$ so we can consider the fibre $f^_(y)$ over y. If we have
two inhabitants $a, b \in f^_(y)$, then we know (by the definition of
fibre) that $f(a) = y = f(b)$, hence by injectivity of $f$, we have $a =
b$ --- if $f^_(y)$ has any points at all, then it has exactly one. It
...
have two inhabitants of $f^_(f(a))$, $a$ and $b$, but since we assumed
...
f^\*(y)$ defines a predicate $\chi\_f : B \to \Omega$, where we have
...
$\rm{Disc}(X) \to _$ as a map $_ \to \set$. The name $\set$ was
...
_category_ $\cE/x$, but only a _set_ $\hom_{\cE}(x, U)$. Objects in a
...
\hom(_, U) \cong \Sets/_ \cong \Sets$. There's an element $b = r\inv(2)
...
shifted the issue one level up: we now have an entire _bicategory_
$\cE/x$, which can't be classified by the mere _category_ of maps
$\hom_{\cE}(-, U)$!
...
$\ty\_\infty : \ty\_\infty$, we could reproduce Russell's paradox, as is
done [here].
...
means "category internal to $\Sets\_\kappa$", as mentioned in the infobox
...
inhabitant of the universe $\ty\_\kappa$, and that the $\hom\_\cC(-,-)$
family is valued in $\Sets\_\kappa$. Our shorthand for the traditional
...
types that are sets, it expresses _refl_exivity of equality. In the
...
in the details that were missing in _Interlude - Basics of
paths_ above. In particular, the
