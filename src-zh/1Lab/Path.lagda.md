...
of _paths_ from $a$ to $b$ in $A$.  There is a constant path $\refl : a
...
defined to be continuous mappings $f : [0,1] \to A$ from the _interval_
...
define _paths_ to be functions $\bI \to A$. We don't have to cut down to
a type of "continuous" functions; instead, we arrange for the interval
_type_ to be so that all functions from it are continuous.
...
A$, we say that it is a path _from_ $f(\iZ)$ _to_ $f(\iO)$. Of course,
...
:::{.definition #path}
However, we can not internally _define_ the type of "functions $f : \bI
...
cubical type theory is equipped with a _primitive_ type of **paths from
$a$ to $b$**.
:::
...
with their own syntax. In Agda, we instead _overload_ the lambda
notation, and function application, so that it can be used for both
functions _and_ paths.

Therefore, while we can define a helper that upgrades any function to a
path, it looks a lot like the identity function; we do not _require_
...
type theory _literature_, which traditionally uses $a \is b$ to mean
...
The easiest path to construct is _reflexivity_, called `refl`{.Agda} for
...
paths _as functions from the interval_, that is, application. Note that,
when we apply $p : a \is b$ to one of the endpoints of the interval, we
_definitionally_ get back the endpoints of the path. Other than the
...
to _dependent_ functions $(i : \bI) \to A(i)$. Indeed we can, and these
...
If we have a _line_ of types $A : \bI \to \ty$, and inhabitants $a :
...
them; and, _over_ this identification, $a$ and $b$ are identical. The
...
connective, being more general, is the actual _primitive_ provided by
...
Cubical Agda, we work in _de Morgan_ cubical type theory. This means
...
We can also invert _dependent_ paths: if $a : A(\iZ)$ and $b : A(\iO)$
...
_definitional_ involutions. Once we implement path _composition_, we'll
...
functions $\bI \to \bI \to A$, with _two_ dimensions: a way of mapping
the _square_ into a type.
...
explore the _other_ ways in which we can lift $n$-cubes to $n+1$ cubes,
...
_one_ dimension, we must either _discard_ one of $i$ or $j$, or find a
way to _combine_ them. Discarding is the easier option. Depending on
what dimension we ignore, we get one of two squares, both of which have
two $p$ faces _across_ from eachother. Note that the square
...
varies top-to-bottom. The `drop-i`{.Agda} square is _constant_ in the
...
we're looking at $p(\iZ) = a$ _not_ varying along the vertical axis, and
