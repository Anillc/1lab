...
some $f^_(a) : A$ and $p : f(f^_a) = a$ (resp $q : f(f^_(b)) = b$) ---
which would establish that $a \equiv b$, as we need, since we have $a =
f(f^_(a)) = f(f^\*(b)) = b$, where the middle equation is by constancy of
...
f x \equiv f y$ _and_ a coherence $\rm{coh}_{x,y,z} : \rm{const}_{x,y}
...
even a _propositional truncation_ of $A$, in that it satisfies the same
